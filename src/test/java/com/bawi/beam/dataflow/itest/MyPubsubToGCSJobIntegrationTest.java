...
//                    .peek(b -> LOGGER.info("filtered blob: {}, {}", b, b.getName()))
            ...
            LOGGER.info("Reading avro files generated by Dataflow. Found {} of blobs ending with .avro", filteredBlobs.size());
            ...
            if (!filteredBlobs.isEmpty()) {
                ...
                if (results.size() == expectedNumMessages) {
                    ...
                    LOGGER.info("Waiting for dataflow job to write all messages to GCS (written {} of {}), attempt {}/{}), retrying in {} secs", results.size(), expectedNumMessages, i, limitRetries, retryDelaySecs);
