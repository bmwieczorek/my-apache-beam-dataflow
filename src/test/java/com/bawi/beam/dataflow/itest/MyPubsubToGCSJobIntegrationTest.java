...
//                    String myMsgAttrValue = generateMessageDuplicates ? MY_MSG_ATTR_VALUE + (msgCounter + 1 - ((msgCounter + 1) % 2)) : MY_MSG_ATTR_VALUE + msgCounter;
    ...
    private List<String> readWithRetriesAvroFileFromGCSGeneratedByDataflowJob(String bucketName, @SuppressWarnings("SameParameterValue") String objectPathPrefix, int expectedNumMessages, int limitRetries) throws InterruptedException {
        ...
        Set<String> blobPaths = new TreeSet<>();
...
//                    .peek(b -> LOGGER.info("filtered blob: {}, {}", b, b.getName()))
            ...
            LOGGER.info("Reading avro files generated by Dataflow. Found {} blobs ending with .avro in path {}", filteredBlobs.size(), objectPathPrefix);
            blobPaths = filteredBlobs.stream().map(BlobInfo::getName).collect(Collectors.toCollection(TreeSet::new));
            ...
            if (!filteredBlobs.isEmpty()) {
                ...
                if (results.size() == expectedNumMessages) {
                    ...
                    LOGGER.info("Waiting for Dataflow to write all messages - written {} of {}, attempt {}/{}, retrying in {} secs", results.size(), expectedNumMessages, i, limitRetries, retryDelaySecs);
                    Thread.sleep(retryDelaySecs * 1000L);
                    outputAvroRecordsAsStrings = results;
                }
            } else {
                LOGGER.info("Waiting for dataflow job to start reading {} message(s) from PubSub, transform and write avro to GCS ... (attempt {}/{})", expectedNumMessages, i, limitRetries);
                Thread.sleep(retryDelaySecs * 1000L);
            }
        }
