...
public class GroupedIntoBatchesFileIOWriteDynamicTest implements Serializable {
    ...
    public void shouldCreateBatches() throws IOException {
        ...
        if (runner == Direct) {
            ...
            Assert.assertTrue(!shard1Lines.isEmpty() && shard1Lines.stream().allMatch(charNumberKVLine -> charNumberKVLine.endsWith("0")));
            ...
            Assert.assertTrue(!shard2Lines.isEmpty() && shard2Lines.stream().allMatch(charNumberKVLine -> charNumberKVLine.endsWith("0")));
        }
    }
