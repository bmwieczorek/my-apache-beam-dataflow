locals {
  ...
  bq_query_select_view_command = "bq query --nouse_legacy_sql 'SELECT * FROM ${local.public_dataset_id}.${google_bigquery_table.view.table_id} WHERE load_timestamp IS NOT NULL'"
  ...
  private_dataset_id = "${local.owner}_private_dataset"
  public_dataset_id = "${local.owner}_public_dataset"

  sa_email = "${local.owner}-sa@${var.project}.iam.gserviceaccount.com"
  other_sa_email = "${local.owner}-other-sa@${var.project}.iam.gserviceaccount.com"
}


resource "google_bigquery_table" "table" {
  project             = var.project
  dataset_id          = local.private_dataset_id
...
resource "google_bigquery_table" "view" {
  project             = var.project
  dataset_id          =  local.public_dataset_id
  table_id            = "${local.owner}_view"
  deletion_protection = false

  view {
    query          = "SELECT id, load_timestamp FROM `${var.project}.${local.private_dataset_id}.${google_bigquery_table.table.table_id}`"
...
resource "google_compute_instance" "vm" {
  depends_on   = [google_bigquery_dataset_access.view_to_dataset_access]
  ...
  metadata_startup_script = <<-EOF
    echo "[${var.owner}-debian] Executing: Executing: \"${local.bq_ls_command}\" using ${local.sa_email} service account with ${join(",", local.project_level_roles)} project role(s) and ${join(",", local.dataset_level_roles)} dataset role(s)"
    ${local.bq_ls_command}
    echo "=========================================="
    echo "[${var.owner}-debian] Executing: Executing: \"${local.bq_query_insert_command}\" using ${local.sa_email} service account with ${join(",", local.project_level_roles)} project role(s) and ${join(",", local.dataset_level_roles)} dataset role(s)"
    ${local.bq_query_insert_command}
    echo "=========================================="
    echo "[${var.owner}-debian] Executing: Executing: \"${local.bq_head_command}\" using ${local.sa_email} service account with ${join(",", local.project_level_roles)} project role(s) and ${join(",", local.dataset_level_roles)} dataset role(s)"
    ${local.bq_head_command}
    echo "=========================================="
    echo "[${var.owner}-debian] Executing: Executing: \"${local.bq_query_select_command}\" using ${local.sa_email} service account with ${join(",", local.project_level_roles)} project role(s) and ${join(",", local.dataset_level_roles)} dataset role(s)"
    ${local.bq_query_select_command}
    echo "=========================================="
    echo "[${var.owner}-debian] Executing: Executing: \"${local.bq_query_select_view_command}\" using ${local.sa_email} service account with ${join(",", local.project_level_roles)} project role(s) and ${join(",", local.dataset_level_roles)} dataset role(s)"
    ${local.bq_query_select_view_command}
    echo "=========================================="
    echo "[${var.owner}-debian] Executing: Executing: \"${local.bq_query_delete_command}\" using ${local.sa_email} service account with ${join(",", local.project_level_roles)} project role(s) and ${join(",", local.dataset_level_roles)} dataset role(s)"
...
resource "google_compute_instance" "vm_other_sa" {
  ...
  metadata_startup_script = <<-EOF
    echo "[${var.owner}-debian-other-sa] Executing: \"${local.bq_query_select_view_command}\" using ${local.other_sa_email} service account with ${join(",", local.project_level_roles)} project role(s) and ${join(",", local.dataset_level_roles)} dataset role(s)"
    ${local.bq_query_select_view_command}
    echo "DONE"
  EOF

  service_account {
    email  = local.other_sa_email
