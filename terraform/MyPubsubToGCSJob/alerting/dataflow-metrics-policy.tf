...
resource "google_monitoring_alert_policy" "job_name_regex_no_reducer_all_series_policy" {
  depends_on = [time_sleep.wait_180_seconds]
  project = var.project
  display_name = "${var.job_name_base}.* 3m job name no reducer all series policy"
  enabled = true
  combiner = "OR"
  notification_channels = [google_monitoring_notification_channel.email.name]
  documentation {
    content = "${var.job_name_base}.* 3m job name no reducer all series documentation"
    mime_type = "text/markdown"
  }
  conditions {
    display_name = "${var.job_name_base}.* 3m job name no reducer all series condition"
    condition_threshold {
      filter        = "metric.type=\"dataflow.googleapis.com/job/elements_produced_count\" resource.type=\"dataflow_job\" resource.label.\"job_name\"=monitoring.regex.full_match(\"${var.job_name_base}.*\") metric.label.\"pcollection\"=\"PubsubIO.Read/MapElements/Map.out0\""
      duration      = "180s" // 3 min time that a time series must violate the threshold to be considered failing
      comparison    = "COMPARISON_LT"
      threshold_value = "0.1"
      trigger {
...
resource "google_monitoring_alert_policy" "job_name_regex_no_reducer_any_series_policy" {
  depends_on = [time_sleep.wait_180_seconds]
  project = var.project
  display_name = "${var.job_name_base}.* 3m job name no reducer any series policy"
  enabled = true
  combiner = "OR"
  notification_channels = [google_monitoring_notification_channel.email.name]
  documentation {
    content = "${var.job_name_base}.* 3m job name no reducer any series documentation"
    mime_type = "text/markdown"
  }
  conditions {
    display_name = "${var.job_name_base}.* 3m job name no reducer any series condition"
    condition_threshold {
      filter        = "metric.type=\"dataflow.googleapis.com/job/elements_produced_count\" resource.type=\"dataflow_job\" resource.label.\"job_name\"=monitoring.regex.full_match(\"${var.job_name_base}.*\") metric.label.\"pcollection\"=\"PubsubIO.Read/MapElements/Map.out0\""
      duration      = "180s" // 3 min time that a time series must violate the threshold to be considered failing
      comparison    = "COMPARISON_LT"
...
resource "google_monitoring_alert_policy" "job_name_regex_sum_reducer_all_series_policy" {
  depends_on = [time_sleep.wait_180_seconds]
  project = var.project
  display_name = "${var.job_name_base}.* 3m job name sum reducer all series policy"
  enabled = true
  combiner = "OR"
  notification_channels = [google_monitoring_notification_channel.email.name]
  documentation {
    content = "${var.job_name_base}.* 3m job name sum reducer all series documentation"
    mime_type = "text/markdown"
  }
  conditions {
    display_name = "${var.job_name_base}.* 3m job name sum reducer all series condition"
    condition_threshold {
      filter        = "metric.type=\"dataflow.googleapis.com/job/elements_produced_count\" resource.type=\"dataflow_job\" resource.label.\"job_name\"=monitoring.regex.full_match(\"${var.job_name_base}.*\") metric.label.\"pcollection\"=\"PubsubIO.Read/MapElements/Map.out0\""
      duration      = "180s" // 3 min time that a time series must violate the threshold to be considered failing
      comparison    = "COMPARISON_LT"
...
resource "google_monitoring_alert_policy" "job_name_regex_sum_reducer_any_series_policy" {
  depends_on = [time_sleep.wait_180_seconds]
  project = var.project
  display_name = "${var.job_name_base}.* 3m job name sum reducer any series policy"
  enabled = true
  combiner = "OR"
  notification_channels = [google_monitoring_notification_channel.email.name]
  documentation {
    content = "${var.job_name_base}.* 3m job name sum reducer any series documentation"
    mime_type = "text/markdown"
  }
  conditions {
    display_name = "${var.job_name_base}.* 3m job name sum reducer any series condition"
    condition_threshold {
      filter        = "metric.type=\"dataflow.googleapis.com/job/elements_produced_count\" resource.type=\"dataflow_job\" resource.label.\"job_name\"=monitoring.regex.full_match(\"${var.job_name_base}.*\") metric.label.\"pcollection\"=\"PubsubIO.Read/MapElements/Map.out0\""
      duration      = "180s" // 3 min time that a time series must violate the threshold to be considered failing
      comparison    = "COMPARISON_LT"
